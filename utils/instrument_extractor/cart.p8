...
function is_valid(note)
	local is_inst = note_has_instrument_bit(note)

	return
		get_note_waveform(note) == wave
		and get_note_volume(note) > 0
		and (
			(inst and is_inst)
			or (not inst and not is_inst)
		) and (
			not effect
			or get_note_effect(note) == effect
		)
end
...
for sfx_idx = 0, 63 do
	...
	for note_idx = 0, 31 do
		...
		elseif
			...
			and is_valid(next_note)
		then
			if get_note_volume(note) > 0 then
				printb("warning: " .. "sfx " .. sfx_idx .. ": note " .. note_idx + 1 .. " slides from invalid note. note " .. note_idx .. " will be copied with volume 0")

				-- zero out note volume
				note = note & 0xf1ff
			end
